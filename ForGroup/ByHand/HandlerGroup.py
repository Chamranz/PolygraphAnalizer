...
import numpy as np
import pandas as pd
from ForGroup.ByHand.GraphBuilderGroup import Building
...
def HandInsertion(page, content):
    ...
    def next_page(e):
        if auto_n_polunom.value == True:
            ...
            if N_polynom.value != '':
                ...
                npolynom = 0
        if N_Task.value == '' or N_resp.value == '':
            page.snack_bar = ft.SnackBar(ft.Text('Введите количество заданий/эпизодов и общее количество респондентов!', font_family=MFont))
            page.snack_bar.open = True
            page.update()
        else:
            if plot_number.value == '':
                TimeInsertion(page, content, int(N_Task.value), int(N_resp.value), plot_number = 1, time_type=time_type.value, npoly=npolynom)
            else:
                if int(plot_number.value) > int(N_resp.value):
                    page.snack_bar = ft.SnackBar(ft.Text('Количество графиков не должно превышать количество респондентов!', font_family=MFont))
                    page.snack_bar.open = True
                    page.update()
                else:
                    TimeInsertion(page, content, int(N_Task.value), int(N_resp.value), plot_number=int(plot_number.value), time_type=time_type.value, npoly=npolynom)
    ...
    N_resp = ft.TextField(label='Введите количество респондентов', keyboard_type=ft.KeyboardType.NUMBER)
    ...
    NextButton = ft.OutlinedButton(text='Далее', on_click=next_page, disabled=False)
    content.controls.clear()
    content.controls.extend([ft.Column([
        ft.Row([
            N_Task, plot_number], alignment=ft.MainAxisAlignment.CENTER),

        ft.Row([N_resp,
                ft.Column([N_polynom,
                           ft.Row([ft.Text(value='Расчитать оптиемальное число волн (по умолчаниею 0 - линия тренда)'), auto_n_polunom])])
...
def TimeInsertion(page, content, n_task, n_resp, plot_number=1, time_type=None, npoly=0):
    ...
    def next_resp(e):
        ...
        timings.append(data_time)
        print(f'Загружены данные по {len(timings)} респондтам из {n_resp}')
        next_button.disabled = True
        upload_button.disabled = False
        page.update()
        if len(timings) == n_resp:
    ...
    def collect_data(e):
        try:
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text('Выберите папку, куда сохранятся графики!', font_family=MFont))
            page.snack_bar.open = True
            page.update()
        else:
            Building(dfs, timings, PN, n_resp, directory, n_task, npoly)
    ...
    next_button = ft.OutlinedButton('Следующий респондент', on_click=next_resp, disabled=True)
    ...
    content.controls.extend([ft.Row([
        ft.Row([save_directory, choose_directory, file_picker, upload_button], alignment=ft.MainAxisAlignment.CENTER),
        ft.OutlinedButton(text='Назад', on_click=back),
        next_button, done_button
    ], alignment=ft.MainAxisAlignment.CENTER)])
    page.update()